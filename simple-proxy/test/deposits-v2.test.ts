...
describe("deposits-v2", () => {
    ...
    beforeEach(async () => {
        ...
        const initData = implementationV2.interface.encodeFunctionData("initialize", [depositFee])

        const proxyFactory = await ethers.getContractFactory("DepositsProxy", ownerAcc)
        depositsProxy = await proxyFactory.deploy(implementationV2.address, initData)
