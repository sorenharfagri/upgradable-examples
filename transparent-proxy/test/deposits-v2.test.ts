...
import { DepositsProxy, DepositsV2, ProxyAdmin } from "../typechain-types";
...
describe("deposits-v2", () => {
    ...
    beforeEach(async () => {
        ...
        const implementationV2Factory = await ethers.getContractFactory("DepositsV2", ownerAcc)
        ...
        const initData = implementationV2.interface.encodeFunctionData("initialize", [depositFee, ownerAcc.address])

        const proxyFactory = await ethers.getContractFactory("DepositsProxy", ownerAcc)
        depositsProxy = await proxyFactory.deploy(implementationV2.address, initData, proxyAdmin.address)
    ...
    it("Non owner cant withdraw profit", async () => {
        ...
        await expect(withdrawProfitTx).to.be.revertedWithCustomError(proxiedImplementation, DepositsV2Errors.NotAnOwner)
    })
